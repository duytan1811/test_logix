...
export class ClassicComponent implements OnInit, OnDestroy {
  ...
  getDateTime() {
    const date = new Date();
    const year = "năm " + new Intl.DateTimeFormat('vi', { year: 'numeric' }).format(date);
    const month = "tháng " + new Intl.DateTimeFormat('vi', { month: 'numeric' }).format(date);
    const day = "ngày " + new Intl.DateTimeFormat('vi', { day: 'numeric' }).format(date);
    var weekday = "Thứ " + (new Intl.DateTimeFormat('vi', { weekday: 'long' }).format(date)).substring(4).toLowerCase();
    if (date.getDay() == 0) {
      weekday = "Chủ nhật"
    }
    return this.currentDate = weekday + ", " + day + " " + month + " " + year;
  }
}
